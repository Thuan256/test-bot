...
module.exports = {
    ...
    run: async (client, message, args) => {
        try {
            ...
            const enable = async (cmd, id, type) => {
                if (cmd === 'disable' || cmd === 'enable') return 'CAN_NOT_ENABLE';

                const commandsData = fs.readFileSync('././data/commands.json', 'utf8');
                const data = JSON.parse(commandsData);

                if (type === 'server') {
                    data[cmd].server = false;
                } else {
                    const enableIndex = data[cmd].enable[type].indexOf(id);
                    if (enableIndex !== -1) return 'ENABLED';

                    const disableIndex = data[cmd].disable[type].indexOf(id);
                    if (disableIndex !== -1) data[cmd].disable[type].splice(disableIndex, 1);

                    data[cmd].enable[type].push(id);
                }
            ...
            if (!cmd) {
                ...
                cmds.forEach(async (cmd) => await enable(cmd.name, id, type));

                const e = new EmbedBuilder()
                    .setDescription(`${client.emoji.greentick} Đã bật nhóm lệnh ${category} ${text}`)
                    .setColor('Green');
                ...
                const result = await enable(key, id, type)

                if (result === 'ENABLED') return error(`Lệnh \`${key}\` đã được bật rồi!`, message);
                else if (result === 'CAN_NOT_ENABLE') return error(`Không thể bật lệnh \`${key}\``, message)

                const e = new EmbedBuilder()
                    .setDescription(`${client.emoji.greentick} Đã bật lệnh \`${key}\` ${text}`)
                    .setColor('Green');
